...
#     --epochs 50 --batch_size_src 128 --batch_size_tgt 128 --lr 1e-3
#
# 特色：
# - 完全動態適應 AP 數量 (自動偵測 CSV 內的 ap 欄位)
# - 透過 AdaptiveAvgPool2d 處理任意大小的 LDPL 2D 特徵圖
...
def get_feature_cols(df: pd.DataFrame):
    """
    動態抓取資料集中的 AP 欄位，不再寫死數量。
    只要欄位名稱開頭是 'ap' 就會被抓進來，並依照數字排序 (ap0, ap1, ap2...)
    """
    ap_cols = [c for c in df.columns if str(c).startswith('ap')]
    # 確保依照數字正確排序，避免 ap10 排在 ap2 前面
...
class iToLocModel(nn.Module):
    def __init__(self, n_classes: int, p_drop=0.5):
        ...
        self.M1 = nn.Sequential(
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            # 關鍵：自動適應池化，不管空間維度變成多少，一律壓縮成 1x1
    ...
    def fingerprint_image_transform(self, rssi):
        # 這裡的 rssi.shape 是 (B, N)，Pytorch 會自動抓取 N 的大小，完全動態！
        d = torch.pow(10.0, (self.R - rssi) / (10.0 * self.eta)) 
        d_j = d.unsqueeze(2) # (B, N, 1)
        d_k = d.unsqueeze(1) # (B, 1, N)
        X = (d_j - d_k) / (d_k + 1e-6)
        return X.unsqueeze(1) # 自動產生 (B, 1, N, N) 影像
...
def main():
    ...
    # --------- Model / Optim ---------
    model = iToLocModel(n_classes=n_classes, p_drop=0.5).to(device)
